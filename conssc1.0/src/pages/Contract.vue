<template>
  <div class="q-ma-sm" >
    <q-btn color="lime-4" class="text-black q-ma-md" style="margin-right:-15px" label="新增合同" @click="addDrawer=true"/>
    <!--合同查询-->
      ...
      <q-card-section>
        <div class="row">
          <div class="col-xs-12 col-lg-3 col-md-6">
            <q-input v-model="queryData.contractNo"
                     dense
                     prefix="合同编号"
                     color="green-5"
                     clearable
                     ...
                     input-class="q-ml-md"
            />
            <q-select v-model="queryData.type"
                     dense
                      use-input
                      fill-input
                      hide-selected
          ...
          <div class="col-xs-12 col-lg-3 col-md-6">
            <q-input v-model.trim="queryData.contractTitle"
                     prefix="合同标题"
                     color="green-5"
                     class="q-pa-sm text-subtitle1"
                     input-class="q-ml-md"
                     clearable dense/>
            <q-input v-model.trim="queryData.companyName"
                     clearable dense
                     prefix="对方名称"
                     color="green-5"
                     input-class="q-ml-md"
          ...
          <div class="col-xs-12 col-lg-3 col-md-6">
            <q-input v-model="queryData.deliveryTime"
                     ...
                     :error="!this.$v.queryData.deliveryTime.maxLength"/>
            <q-select
              use-input
              fill-input
              hide-selected
          ...
          <div class="col-xs-12 col-lg-3 col-md-6">
            <q-select v-model="queryData.status"
                      prefix="合同状态" color="green-5"
                     ...
                     class="q-pa-sm text-subtitle1"
                      ...
                      emit-value
                    />
            <q-input v-model="queryData.remark" prefix="合同备注" color="green-5" input-class="q-ml-md"
    ...
    <q-card class="q-mt-lg q-ma-md" square bordered flat>
      ...
      <q-table
        ...
        :loading="tbLoading"
        :data="dataList"
        :pagination.sync="contractPagination"
        hide-pagination
        separator="cell"
      >
        ...
        <template v-slot:body="props">
          <q-tr>
            ...
            <q-td key="contractTitle"  class="sticky-td bg-grey-2">
            ...
            <q-td key="invoiceNum" class="text-right" :class="props.row.totalNum===props.row.invoiceNum?'':'text-red text-bold'">
              ...
              <q-popup-edit v-model="rowData.invoiceNum"
                            label-set="保存"
                            color="green"
                            touch-position
                            anchor="top left"
                            :title="rowData.contractNo"
                            @before-show="getRowData(props)"
                            buttons
                            @save="sendEditedData">
                <q-input v-model="rowData.invoiceNum" dense autofocus color="green"/>
              </q-popup-edit>
            </q-td>
            ...
            <q-td >
              <q-icon class="q-pa-xs" color="primary" size="xs" flat @click="doContractSign(props)"><strong>审</strong>
                <q-tooltip>合同审批</q-tooltip>
              </q-icon>
              <q-icon class="q-pa-xs" color="green" size="xs" flat @click="doAccept(props)"><strong>验</strong>
                <q-tooltip>去验收</q-tooltip>
              </q-icon>
              <q-icon class="q-pa-xs" color="pink" size="xs" flat><strong>付</strong>
                <q-tooltip>去付款</q-tooltip>
              </q-icon>
            </q-td>
            <q-td>
            ...
            <q-td key="signTime" class="text-center">
              {{ props.row.signTime }}
            </q-td>
            <q-td key="deliveryTime" class="text-center"
                  :class="!props.row.hasOwnProperty('deliveryTime')? '' :(props.row.status===1?'':(mentionDate(props.row.deliveryTime)<0?'text-red text-bold':'bg-purple text-white'))">
...
import '../util/filter'
import { rowsPerPage, addDrawer, editDialog, crud } from '../components/mixin/baseMixin'
import printJS from 'print-js'
import { printCss } from '../css/print'
import { mentionDate } from '../util/dataTimeHandler'
export default {
  name: 'Contract',
  components: { NumPerPage, Tips },
  mixins: [rowsPerPage, addDrawer, editDialog, crud],
  data () {
    return {
      editDialog: false,
      acceptDialog: false,
  ...
  computed: {
    ...
    contractPagination: {
      get () {
        return this.$store.state.Contract.contractPagination
      },
      set () {
      }
    }
  ...
  methods: {
    ...
    ...mapActions('Contract',
      [
        'contractNewDateAction',
        'contractTrashDataAction',
        'contractSendEditedAction',
        'contractModuleDataAction',
        'counterpartySelectAction',
        'detailForContractAction'
      ]),
    ...mapActions('Project',
      [
    ...
    /* 获取待查询的数据 */
    onRequest () {
      this.contractModuleDataAction({
        pagination: this.contractPagination,
